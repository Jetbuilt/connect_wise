...
module ConnectWise
  class Member
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::Member` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::Member`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect unless self.class.attribute_map.key?(k.to_sym)
      ...
      self.restrict_default_sales_territory_flag = attributes[:restrict_default_sales_territory_flag] if attributes.key?(:restrict_default_sales_territory_flag)
      ...
      self.enable_ldap_authentication_flag = attributes[:enable_ldap_authentication_flag] if attributes.key?(:enable_ldap_authentication_flag)
    ...
    def valid?
      ...
      phone_integration_type_validator = EnumAttributeValidator.new('String', %w[TAPI SKYPE NONE])
    ...
    def phone_integration_type=(phone_integration_type)
      validator = EnumAttributeValidator.new('String', %w[TAPI SKYPE NONE])
    ...
    def hash
      [identifier, first_name, last_name, license_class, default_email, default_phone, hire_date, company_activity_tab_format, invoice_time_tab_format, invoice_screen_default_tab_format, invoicing_display_options, agreement_invoicing_display_options, id, password, middle_initial, title, report_card, disable_online_flag, enable_mobile_flag, type, employee_identifer, vendor_number, notes, time_zone, country, service_board_team_ids, enable_mobile_gps_flag, inactive_date, inactive_flag, last_login, photo, partner_portal_flag, client_id, sts_user_admin_url, token, toast_notification_flag, member_personas, office365, office_email, office_phone, office_extension, mobile_email, mobile_phone, mobile_extension, home_email, home_phone, home_extension, primary_email, security_role, admin_flag, structure_level, security_location, default_location, default_department, reports_to, restrict_location_flag, restrict_department_flag, work_role, work_type, time_approver, expense_approver, billable_forecast, daily_capacity, hourly_cost, hourly_rate, include_in_utilization_reporting_flag, require_expense_entry_flag, require_time_sheet_entry_flag, require_start_and_end_time_on_time_entry_flag, allow_in_cell_entry_on_time_sheet, enter_time_against_company_flag, allow_expenses_entered_against_companies_flag, time_reminder_email_flag, days_tolerance, minimum_hours, time_sheet_start_date, service_default_location, service_default_department, service_default_board, restrict_service_default_location_flag, restrict_service_default_department_flag, excluded_service_board_ids, project_default_location, project_default_department, project_default_board, restrict_project_default_location_flag, restrict_project_default_department_flag, excluded_project_board_ids, schedule_default_location, schedule_default_department, schedule_capacity, service_location, restrict_schedule_flag, hide_member_in_dispatch_portal_flag, calendar, sales_default_location, restrict_default_sales_territory_flag, warehouse, warehouse_bin, restrict_default_warehouse_flag, restrict_default_warehouse_bin_flag, mapi_name, calendar_sync_integration_flag, enable_ldap_authentication_flag, ldap_configuration, ldap_user_name, authentication_service_type, timebased_one_time_password_activated, sso_settings, auto_start_stopwatch, auto_popup_quick_notes_with_stopwatch, signature, global_search_default_ticket_filter, global_search_default_sort, phone_source, phone_integration_type, use_browser_language_flag, _info, copy_pod_layouts, copy_shared_default_views, copy_column_layouts_and_filters, from_member_rec_id, custom_fields].hash
    end
