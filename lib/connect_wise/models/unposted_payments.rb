...
module ConnectWise
  class UnpostedPayments
    attr_accessor :id, :type, :source, :invoice, :amount, :payment_date, :applied_by, :_info, :wise_pay_payment, :payment_sync_status, :payment_sync_date, :payment_account, :a_r_payment_account
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::UnpostedPayments` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::UnpostedPayments`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)
    ...
    def hash
      [id, type, source, invoice, amount, payment_date, applied_by, _info, wise_pay_payment, payment_sync_status, payment_sync_date, payment_account, a_r_payment_account].hash
    end
