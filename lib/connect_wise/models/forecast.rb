...
module ConnectWise
  class Forecast
    attr_accessor :id, :forecast_items, :product_revenue, :service_revenue, :agreement_revenue, :time_revenue,
                  :expense_revenue, :forecast_revenue_totals, :inclusive_revenue_totals, :recurring_total, :won_revenue, :lost_revenue, :open_revenue, :other_revenue1, :other_revenue2, :sales_tax_revenue, :forecast_total_with_taxes, :expected_probability, :tax_code, :billing_terms, :currency, :_info
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError,
              'The input argument (attributes) must be a hash in `ConnectWise::Forecast` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless acceptable_attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `ConnectWise::Forecast`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
    ...
    def hash
      [id, forecast_items, product_revenue, service_revenue, agreement_revenue, time_revenue, expense_revenue,
       forecast_revenue_totals, inclusive_revenue_totals, recurring_total, won_revenue, lost_revenue, open_revenue, other_revenue1, other_revenue2, sales_tax_revenue, forecast_total_with_taxes, expected_probability, tax_code, billing_terms, currency, _info].hash
    end
