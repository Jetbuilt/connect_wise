  ...
  class CompanyStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConnectWise::CompanyStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::CompanyStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'disallow_saving_flag')
        self.disallow_saving_flag = attributes[:'disallow_saving_flag']
      end
      ...
      if attributes.key?(:'custom_note_flag')
        self.custom_note_flag = attributes[:'custom_note_flag']
      end

      if attributes.key?(:'cancel_open_tracks_flag')
        self.cancel_open_tracks_flag = attributes[:'cancel_open_tracks_flag']
      end
