...
module ConnectWise
  class OpportunityToSalesOrderConversion
    attr_accessor :sales_order_id, :name, :include_all_notes_flag, :include_all_documents_flag, :include_all_products_flag, :include_note_ids, :include_document_ids, :include_product_ids
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::OpportunityToSalesOrderConversion` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::OpportunityToSalesOrderConversion`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect unless self.class.attribute_map.key?(k.to_sym)
