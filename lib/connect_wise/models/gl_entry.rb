...
module ConnectWise
  class GLEntry
    attr_accessor :id, :type, :amount, :cost, :is_batched, :_info
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::GLEntry` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::GLEntry`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)
      ...
      self.is_batched = attributes[:is_batched] if attributes.key?(:is_batched)
    ...
    def hash
      [id, type, amount, description, segment1, segment2, segment3, segment4, segment5, segment6, segment7, segment8, segment9, segment10, product_id, cost, sales_code, inventory, cogs1, cogs2, cogs3, cogs4, cogs5, cogs6, cogs7, cogs8, cogs9, cogs10, is_batched, _info].hash
    end
