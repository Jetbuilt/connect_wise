...
module ConnectWise
  class ConnectWiseHostedSetup
    ...
    attr_accessor :type, :id, :pod_height, :toolbar_button_dialog_height, :toolbar_button_dialog_width, :toolbar_button_icon_document_id, :disabled_flag, :location_ids, :locations_enabled_flag, :created_by, :date_created, :_info
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::ConnectWiseHostedSetup` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::ConnectWiseHostedSetup`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)

        h[k.to_sym] = v
      end

      self.screen_id = (attributes[:screen_id] if attributes.key?(:screen_id))
      ...
      self.pod_height = attributes[:pod_height] if attributes.key?(:pod_height)

      self.toolbar_button_dialog_height = attributes[:toolbar_button_dialog_height] if attributes.key?(:toolbar_button_dialog_height)

      self.toolbar_button_dialog_width = attributes[:toolbar_button_dialog_width] if attributes.key?(:toolbar_button_dialog_width)

      self.toolbar_button_text = attributes[:toolbar_button_text] if attributes.key?(:toolbar_button_text)

      self.toolbar_button_tool_tip = attributes[:toolbar_button_tool_tip] if attributes.key?(:toolbar_button_tool_tip)

      self.toolbar_button_icon_document_id = attributes[:toolbar_button_icon_document_id] if attributes.key?(:toolbar_button_icon_document_id)

      self.disabled_flag = attributes[:disabled_flag] if attributes.key?(:disabled_flag)

      if attributes.key?(:location_ids) && (value = attributes[:location_ids]).is_a?(Array)
        self.location_ids = value
      end

      self.locations_enabled_flag = attributes[:locations_enabled_flag] if attributes.key?(:locations_enabled_flag)
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', %w[Tab Pod ToolbarButton])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', %w[Tab Pod ToolbarButton])
    ...
    def ==(other)
      ...
      self.class == other.class &&
        ...
        origin == other.origin &&
        pod_height == other.pod_height &&
        toolbar_button_dialog_height == other.toolbar_button_dialog_height &&
        toolbar_button_dialog_width == other.toolbar_button_dialog_width &&
        toolbar_button_text == other.toolbar_button_text &&
        toolbar_button_tool_tip == other.toolbar_button_tool_tip &&
        toolbar_button_icon_document_id == other.toolbar_button_icon_document_id &&
        disabled_flag == other.disabled_flag &&
        location_ids == other.location_ids &&
        locations_enabled_flag == other.locations_enabled_flag &&
