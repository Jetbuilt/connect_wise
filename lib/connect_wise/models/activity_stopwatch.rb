...
module ConnectWise
  class ActivityStopwatch
    attr_accessor :activity_id, :member, :status, :activity_mobile_guid, :agreement, :billable_option, :business_unit_id, :date_entered, :end_time, :id, :internal_notes, :location_id, :mobile_guid, :start_time, :total_pause_time, :work_role, :work_type, :_info
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::ActivityStopwatch` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::ActivityStopwatch`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)

        h[k.to_sym] = v
      end

      self.activity_id = (attributes[:activity_id] if attributes.key?(:activity_id))

      self.member = (attributes[:member] if attributes.key?(:member))

      self.status = (attributes[:status] if attributes.key?(:status))

      self.activity_mobile_guid = attributes[:activity_mobile_guid] if attributes.key?(:activity_mobile_guid)
    ...
    def ==(other)
      ...
      self.class == other.class &&
        ...
        member == other.member &&
        status == other.status &&
        activity_mobile_guid == other.activity_mobile_guid &&
    ...
    def hash
      [activity_id, member, status, activity_mobile_guid, agreement, billable_option, business_unit_id, date_entered, end_time, id, internal_notes, location_id, mobile_guid, notes, start_time, total_pause_time, work_role, work_type, _info].hash
    end
