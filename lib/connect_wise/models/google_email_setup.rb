...
module ConnectWise
  class GoogleEmailSetup
    ...
    attr_accessor :id, :inactive_flag, :email_connector, :_info
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::GoogleEmailSetup` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::GoogleEmailSetup`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)
      ...
      self.private_key = attributes[:private_key] if attributes.key?(:private_key)
    ...
    def hash
      [name, username, inbox_folder, processed_folder, failed_folder, id, client_id, private_key, inactive_flag, email_connector, _info].hash
    end
