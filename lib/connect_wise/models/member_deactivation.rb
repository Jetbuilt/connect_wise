...
module ConnectWise
  class MemberDeactivation
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::MemberDeactivation` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::MemberDeactivation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect unless self.class.attribute_map.key?(k.to_sym)
      ...
      self.project_expense_approver = attributes[:project_expense_approver] if attributes.key?(:project_expense_approver)
      ...
      self.my_company_service_manager = attributes[:my_company_service_manager] if attributes.key?(:my_company_service_manager)

      self.my_company_duty_manager_role = attributes[:my_company_duty_manager_role] if attributes.key?(:my_company_duty_manager_role)
