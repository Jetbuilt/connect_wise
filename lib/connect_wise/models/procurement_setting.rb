  ...
  class ProcurementSetting
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError,
              'The input argument (attributes) must be a hash in `ConnectWise::ProcurementSetting` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless acceptable_attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `ConnectWise::ProcurementSetting`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end

        h[k.to_sym] = v
      end

      self.starting_purchase_order_num = if attributes.key?(:starting_purchase_order_num)
                                           attributes[:starting_purchase_order_num]
                                         end
      ...
      if attributes.key?(:notification_for_changes_in_shipping_info_flag)
        self.notification_for_changes_in_shipping_info_flag = attributes[:notification_for_changes_in_shipping_info_flag]
      end
    ...
    def costing_method=(costing_method)
      ...
      unless validator.valid?(costing_method)
        raise ArgumentError, "invalid value for \"costing_method\", must be one of #{validator.allowable_values}."
      end
    ...
    def prefix_suffix_type=(prefix_suffix_type)
      ...
      unless validator.valid?(prefix_suffix_type)
        raise ArgumentError, "invalid value for \"prefix_suffix_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [starting_purchase_order_num, costing_method, id, purchase_order_prefix, purchase_order_suffix,
       prefix_suffix_type, disable_cost_updates_flag, disable_negative_inventory_flag, auto_close_purchase_order_flag, auto_close_purchase_order_item_flag, auto_approve_purchase_order_flag, tax_purchase_order_flag, tax_freight_flag, use_vendor_tax_code_flag, num_decimal_places, disable_auto_pick_flag, default_product_taxable_flag, eori_number, _info, notification_for_changes_in_shipping_info_flag, shipping_info_notification_email].hash
    end
