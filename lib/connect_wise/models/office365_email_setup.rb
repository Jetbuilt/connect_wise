...
module ConnectWise
  class Office365EmailSetup
    ...
    attr_accessor :id, :authorized_flag, :inactive_flag, :source, :use_existing_tenant_flag, :existing_tenant, :email_connector, :_info
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::Office365EmailSetup` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::Office365EmailSetup`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)
      ...
      self.use_existing_tenant_flag = attributes[:use_existing_tenant_flag] if attributes.key?(:use_existing_tenant_flag)
    ...
    def list_invalid_properties
      ...
      invalid_properties.push('invalid value for "processed_folder", processed_folder cannot be nil.') if @processed_folder.nil?
    ...
    def hash
      [name, inbox_folder, processed_folder, failed_folder, id, username, tenant_id, client_id, client_secret, authorized_flag, inactive_flag, source, use_existing_tenant_flag, existing_tenant, email_connector, _info].hash
    end
