...
module ConnectWise
  class OpportunityToServiceTicketConversion
    attr_accessor :ticket_id, :summary, :include_all_notes_flag, :include_all_documents_flag, :include_all_products_flag, :include_note_ids, :include_document_ids, :include_product_ids
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::OpportunityToServiceTicketConversion` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::OpportunityToServiceTicketConversion`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)
