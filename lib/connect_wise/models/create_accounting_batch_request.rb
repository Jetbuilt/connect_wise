...
module ConnectWise
  class CreateAccountingBatchRequest
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::CreateAccountingBatchRequest` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::CreateAccountingBatchRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect unless self.class.attribute_map.key?(k.to_sym)

        h[k.to_sym] = v
      end

      if attributes.key?(:processed_record_ids) && (value = attributes[:processed_record_ids]).is_a?(Array)
    ...
    def list_invalid_properties
      invalid_properties = []
      invalid_properties.push('invalid value for "processed_record_ids", processed_record_ids cannot be nil.') if @processed_record_ids.nil?
