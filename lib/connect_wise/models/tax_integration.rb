...
module ConnectWise
  class TaxIntegration
    attr_accessor :tax_integration_type, :id, :enabled_flag, :commit_transactions_flag, :sales_invoice_flag,
                  :accounting_integration_flag, :tax_line_flag, :_info
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError,
              'The input argument (attributes) must be a hash in `ConnectWise::TaxIntegration` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless acceptable_attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `ConnectWise::TaxIntegration`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
    ...
    def tax_integration_type=(tax_integration_type)
      ...
      unless validator.valid?(tax_integration_type)
        raise ArgumentError, "invalid value for \"tax_integration_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [tax_integration_type, id, account_number, license_key, service_url, company_code, time_tax_code,
       expense_tax_code, product_tax_code, invoice_amount_tax_code, enabled_flag, commit_transactions_flag, sales_invoice_flag, freight_tax_code, accounting_integration_flag, tax_line_flag, _info].hash
    end
