...
module ConnectWise
  class UnpostedExpense
    attr_accessor :id, :location_id, :department_id, :company, :account_number, :credit_account, :expense_detail_id,
                  :expense_type, :classification, :gl_type, :member, :date_expense, :charge_code, :charge_description, :in_policy, :payment_method, :currency, :total, :billable_amount, :non_billable_amount, :agreement, :agreement_amount_covered, :ticket, :project, :project_phase, :tax_code, :item_taxable_flag, :sales_tax_amount, :state_tax_xref, :state_tax_amount, :county_tax_xref, :county_tax_amount, :city_tax_xref, :city_tax_amount, :country_tax_xref, :country_tax_amount, :composite_tax_xref, :composite_tax_amount, :level_six_tax_xref, :level_six_tax_amount, :date_closed, :_info
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError,
              'The input argument (attributes) must be a hash in `ConnectWise::UnpostedExpense` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless acceptable_attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `ConnectWise::UnpostedExpense`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
    ...
    def valid?
      ...
      gl_type_validator = EnumAttributeValidator.new('String',
                                                     %w[AP AR EE EI EO IA IT P PF R RA RD RE RP ST SD ET FT PT WP
                                                        WR])
    ...
    def classification=(classification)
      ...
      unless validator.valid?(classification)
        raise ArgumentError, "invalid value for \"classification\", must be one of #{validator.allowable_values}."
      end
    ...
    def gl_type=(gl_type)
      ...
      unless validator.valid?(gl_type)
        raise ArgumentError, "invalid value for \"gl_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [id, location_id, department_id, company, account_number, credit_account, expense_detail_id, expense_type,
       classification, gl_type, member, date_expense, charge_code, charge_description, in_policy, payment_method, currency, total, billable_amount, non_billable_amount, agreement, agreement_amount_covered, ticket, project, project_phase, tax_code, avalara_tax_flag, item_taxable_flag, sales_tax_amount, state_tax_flag, state_tax_xref, state_tax_amount, county_tax_flag, county_tax_xref, county_tax_amount, city_tax_flag, city_tax_xref, city_tax_amount, country_tax_flag, country_tax_xref, country_tax_amount, composite_tax_flag, composite_tax_xref, composite_tax_amount, level_six_tax_flag, level_six_tax_xref, level_six_tax_amount, date_closed, _info].hash
    end
