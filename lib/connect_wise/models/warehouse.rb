  ...
  class Warehouse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConnectWise::Warehouse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::Warehouse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'locked_flag')
        self.locked_flag = attributes[:'locked_flag']
      end
    ...
    def hash
      [name, id, company, location, contact, department, manager, site, location_xref, location_default_flag, overall_default_flag, inactive_flag, locked_flag, currency, _info].hash
    end
