...
module ConnectWise
  class TicketNote
    attr_accessor :id, :ticket_id, :text, :detail_description_flag, :internal_analysis_flag, :resolution_flag, :issue_flag, :member, :contact, :customer_updated_flag, :process_notifications, :internal_flag, :external_flag, :_info
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::TicketNote` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::TicketNote`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)
