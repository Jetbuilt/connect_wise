  ...
  class ChargeCode
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConnectWise::ChargeCode` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::ChargeCode`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'expense_entry_flag')
        self.expense_entry_flag = attributes[:'expense_entry_flag']
      end
      ...
      if attributes.key?(:'time_entry_flag')
        self.time_entry_flag = attributes[:'time_entry_flag']
      end
      ...
      if attributes.key?(:'integration_xref')
        self.integration_xref = attributes[:'integration_xref']
      end
    ...
    def hash
      [name, id, company, location, department, bill_time, expense_entry_flag, allow_all_expense_type_flag, time_entry_flag, work_type, work_role, integration_xref, expense_type_ids, _info].hash
    end
