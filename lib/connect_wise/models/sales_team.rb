  ...
  class SalesTeam
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ConnectWise::SalesTeam` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::SalesTeam`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'sales_team_identifier')
        self.sales_team_identifier = attributes[:'sales_team_identifier']
      end

      if attributes.key?(:'sales_team_description')
        self.sales_team_description = attributes[:'sales_team_description']
      end
      ...
      if attributes.key?(:'sales_team_location')
        self.sales_team_location = attributes[:'sales_team_location']
      end
