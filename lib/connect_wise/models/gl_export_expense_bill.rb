...
module ConnectWise
  class GLExportExpenseBill
    attr_accessor :id, :document_date, :document_type, :document_number, :memo, :gl_class, :ap_account_number, :member, :vendor_number, :currency, :total, :detail
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::GLExportExpenseBill` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::GLExportExpenseBill`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect unless self.class.attribute_map.key?(k.to_sym)
      ...
      return unless (value = attributes[:detail]).is_a?(Array)
