...
module ConnectWise
  class ProductRecurring
    attr_accessor :recurring_revenue, :recurring_cost, :start_date, :bill_cycle_id, :bill_cycle, :cycles, :cycle_type, :agreement_type
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::ProductRecurring` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::ProductRecurring`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)
      ...
      self.cycle_type = attributes[:cycle_type] if attributes.key?(:cycle_type)

      return unless attributes.key?(:agreement_type)
    ...
    def valid?
      ...
      cycle_type_validator = EnumAttributeValidator.new('String', %w[ContractYear CalendarYear])
    ...
    def cycle_type=(cycle_type)
      ...
      raise ArgumentError, "invalid value for \"cycle_type\", must be one of #{validator.allowable_values}." unless validator.valid?(cycle_type)
    ...
    def hash
      [recurring_revenue, recurring_cost, start_date, end_date, bill_cycle_id, bill_cycle, cycles, cycle_type, agreement_type].hash
    end
