...
module ConnectWise
  class TrackAction
    attr_accessor :notify_type, :id, :service_template, :specific_member_to, :specific_member_from, :notes,
                  :activity_type, :activity_status, :company_status, :track, :attached_track, :group, :cc_contact, :bcc_contact, :days_to_execute, :notify_who, :notify_from, :_info
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError,
              'The input argument (attributes) must be a hash in `ConnectWise::TrackAction` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless acceptable_attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `ConnectWise::TrackAction`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
    ...
    def valid?
      ...
      notify_type_validator = EnumAttributeValidator.new('String',
                                                         %w[CreateActivity SendEmail AddToGroup AttachTrack
                                                            ChangeCompanyStatus CreateServiceTicket])
    ...
    def notify_type=(notify_type)
      validator = EnumAttributeValidator.new('String',
                                             %w[CreateActivity SendEmail AddToGroup AttachTrack ChangeCompanyStatus
                                                CreateServiceTicket])
      unless validator.valid?(notify_type)
        raise ArgumentError, "invalid value for \"notify_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [notify_type, id, service_template, specific_member_to, email_recipient, specific_member_from, email_from,
       subject, notes, activity_type, activity_status, company_status, track, attached_track, group, cc_contact, bcc_contact, days_to_execute, notify_who, notify_from, _info].hash
    end
