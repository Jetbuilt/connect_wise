...
module ConnectWise
  class TimeExpense
    attr_accessor :internal_company, :id, :tier1_approval_flag, :tier2_approval_flag, :disable_time_entry_flag, :require_time_note_flag, :require_expense_note_flag, :rounding_factor, :invoice_start, :default_special_invoice_type, :_info
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::TimeExpense` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::TimeExpense`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)
      ...
      self.require_expense_note_flag = attributes[:require_expense_note_flag] if attributes.key?(:require_expense_note_flag)
      ...
      self.default_special_invoice_type = attributes[:default_special_invoice_type] if attributes.key?(:default_special_invoice_type)
    ...
    def list_invalid_properties
      ...
      invalid_properties = []
      invalid_properties.push('invalid value for "internal_company", internal_company cannot be nil.') if @internal_company.nil?
    ...
    def valid?
      ...
      default_special_invoice_type_validator = EnumAttributeValidator.new('String', %w[Agreement CreditMemo DownPayment Miscellaneous Progress Standard])
    ...
    def default_special_invoice_type=(default_special_invoice_type)
      ...
      raise ArgumentError, "invalid value for \"default_special_invoice_type\", must be one of #{validator.allowable_values}." unless validator.valid?(default_special_invoice_type)
    ...
    def hash
      [internal_company, id, tier1_approval_flag, tier2_approval_flag, disable_time_entry_flag, require_time_note_flag, require_expense_note_flag, rounding_factor, invoice_start, default_special_invoice_type, _info].hash
    end
