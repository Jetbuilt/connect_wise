...
module ConnectWise
  class ForecastItem
    attr_accessor :opportunity, :status, :forecast_type, :id, :quantity, :catalog_item, :product_description,
                  :product_class, :revenue, :cost, :margin, :percentage, :include_flag, :quote_werks_quantity, :link_flag, :recurring_revenue, :recurring_cost, :recurring_date_start, :recurring_date_end, :bill_cycle, :cycle_basis, :cycles, :recurring_flag, :sequence_number, :sub_number, :taxable_flag, :_info
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError,
              'The input argument (attributes) must be a hash in `ConnectWise::ForecastItem` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless acceptable_attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `ConnectWise::ForecastItem`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
    ...
    def valid?
      ...
      forecast_type_validator = EnumAttributeValidator.new('String',
                                                           %w[Other1 Other2 Agreement Product Service])
    ...
    def forecast_type=(forecast_type)
      ...
      unless validator.valid?(forecast_type)
        raise ArgumentError, "invalid value for \"forecast_type\", must be one of #{validator.allowable_values}."
      end
    ...
    def hash
      [opportunity, status, forecast_type, id, forecast_description, quantity, catalog_item, product_description,
       product_class, revenue, cost, margin, percentage, include_flag, quote_werks_doc_no, quote_werks_doc_name, quote_werks_quantity, link_flag, recurring_revenue, recurring_cost, recurring_date_start, recurring_date_end, bill_cycle, cycle_basis, cycles, recurring_flag, sequence_number, sub_number, taxable_flag, _info].hash
    end
