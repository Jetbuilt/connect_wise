...
module ConnectWise
  class ServiceTask
    attr_accessor :id, :ticket_id, :notes, :closed_flag, :priority, :schedule, :code, :member, :resolution, :child_schedule_action, :child_ticket_id, :summary, :_info
    ...
    def initialize(attributes = {})
      raise ArgumentError, 'The input argument (attributes) must be a hash in `ConnectWise::ServiceTask` initialize method' unless attributes.is_a?(Hash)

      # check to see if the attribute exists and convert string to symbol for hash key
      acceptable_attribute_map = self.class.acceptable_attribute_map
      attributes = attributes.each_with_object({}) do |(k, v), h|
        raise ArgumentError, "`#{k}` is not a valid attribute in `ConnectWise::ServiceTask`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect unless acceptable_attribute_map.key?(k.to_sym)
    ...
    def child_schedule_action=(child_schedule_action)
      validator = EnumAttributeValidator.new('String', %w[Transfer Delete Done])
      raise ArgumentError, "invalid value for \"child_schedule_action\", must be one of #{validator.allowable_values}." unless validator.valid?(child_schedule_action)
    ...
    def hash
      [id, ticket_id, notes, closed_flag, priority, schedule, code, member, resolution, child_schedule_action, child_ticket_id, summary, _info].hash
    end
